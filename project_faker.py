...
departments_data = []
department_names = ["Computer Science", "Engineering", "Business", "Biology", "Chemistry", "Physics", "Mathematics", "Law", "Arts", "Education"]
for department in department_names:  # Creating department records
    departments_data.append([
        fake.unique.random_number(digits=6),  # DepartmentID (Primary Key)
        department,                           # Department Name
        fake.sentence(nb_words=5),             # Description of Department
        fake.random_element(elements=("Active", "Inactive")),  # Status of the Department
        fake.date_this_century(),             # Establishment Date
    ])

departments_df = pd.DataFrame(departments_data, columns=[
    "DepartmentID", "DepartmentName", "Description", "Status", "EstablishmentDate"])
